(function($, undefined) {
	...
	jPlayerPlaylist.prototype = {
		...
		_createListItem: function(media) {
			...
			var rate='';
			var count=0;
		    var ratingBar = '<span class="rating-bar"></span>';
			var ratingBarOn = '<span class="rating-bar-on"></span>';
			
			var listItem = "<li><div class='list_content'>";
			listItem += "<a style=\"float:right;\" href='javascript:;' class='" + this.options.playlistOptions.removeItemClass + "' style>&times;</a>";
            listItem +="<a href='javascript:;' class='" + this.options.playlistOptions.itemClass + "' tabindex='1'><div class='song'>" + media.title+"</div><div class='rating'><div class='ratingDiv'>";
			if(localStorage[media.title]){
                var info=jQuery.parseJSON(localStorage[media.title]);
				    count=info.playCount;
					if(info.heart){
					    var heart=info.heart;
					}else{ 
					    var heart=0;
					}
			    var rating_num = Math.ceil((count/50)*10);
			}else{
			    rating_num=0;
			}
			for (var i = 0; i < 10; i++){
			...
			listItem +=rate+"</div><div class='playCount'>"+count+"</div></div></a><div class='heartArea'><a href='javascript:void(0)' ";
			if(heart){
			     listItem += "class='heart heart-on'";
			}else{
			     listItem += "class='heart'";
			}
			listItem +="onclick=\"heart(this,'"+escape(media.title)+"')\" >&hearts;</a></div></div></li>";
			return listItem;
		},
		...
		_highlight: function(index) {
			if(this.playlist.length && index !== undefined) {
				...
				var currentMp3 = $(".jp-playlist-current").find("div.song")[0].innerHTML;
				if(localStorage[currentMp3]){
					var info=jQuery.parseJSON(localStorage[currentMp3]);
					    if(info.heart){
						   var $_heart=info.heart;
						}else{
						   var $_heart=0;
						}
						...
						localStorage.setItem("allPlayCount",parseInt(localStorage.getItem("allPlayCount"))+1);
				}
				var song = $(".jp-playlist-current").find("div.song")[0].innerHTML;
				$("#musicName").html(song);
				$.post("./ajax/lyric.php",{query:song},function(result){
                     if(result!='null'){
						 $("#lyric").html(result);
		             }else{
			             $("#lyric").html('none');
				...
				$(".jp-playlist-current .playCount").html(parseInt($(".jp-playlist-current .playCount")[0].innerHTML)+1);
			}
		},
